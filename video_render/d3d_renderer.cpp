...
D3dRenderer::D3dRenderer(size_t width, size_t height,
    ...
    : cvideo_renderer(track_to_render)
	, width_(width),
      height_(height),
	  fps_(0),
      hwnd_(NULL),
...
void D3dRenderer::OnFrame(const webrtc::VideoFrame& frame) {
	...
	const webrtc::VideoFrame*videoFrame = &frame;
  ...
  ConvertFromI420(video_frame, webrtc::VideoType::kARGB, 0, static_cast<uint8_t*>(lock_rect.pBits));
