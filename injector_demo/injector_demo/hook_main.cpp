...
{
	WORD process_id = 30340;
	WCHAR load_path1[MAX_PATH] = L"";
	WCHAR load_path2[MAX_PATH] = L"";
	WCHAR load_path3[MAX_PATH] = L"";
	GetCurrentDirectoryW(MAX_PATH,  load_path1);
	GetCurrentDirectoryW(MAX_PATH, load_path2);
	GetCurrentDirectoryW(MAX_PATH, load_path3);
	wcscat_s(load_path1, L"\\runtime\\");
	wcscat_s(load_path2, L"\\runtime\\");
	wcscat_s(load_path3, L"\\runtime\\");
	wcscat_s(load_path1,   L"rtc.dll");
	wcscat_s(load_path2, L"cloud_rendering.dll");
	wcscat_s(load_path3, L"capture_hook64.dll");

	if (GetFileAttributesW( load_path1) == INVALID_FILE_ATTRIBUTES)
	{
		wprintf(L"\nFailed to find hook_test64 at \"%s\"!\n",  load_path1);
		return ERROR_FILE_NOT_FOUND;
	}
	inject_library_full(process_id, load_path1);
	if (GetFileAttributesW(load_path2) == INVALID_FILE_ATTRIBUTES)
	{
		wprintf(L"\nFailed to find hook_test64 at \"%s\"!\n", load_path2);
		return ERROR_FILE_NOT_FOUND;
	}
	inject_library_full(process_id, load_path2);
	if (GetFileAttributesW(load_path3) == INVALID_FILE_ATTRIBUTES)
	{
		wprintf(L"\nFailed to find hook_test64 at \"%s\"!\n", load_path3);
		return ERROR_FILE_NOT_FOUND;
	}
	inject_library_full(process_id, load_path3);
