...
static void update_acl_for_uwp(LPWSTR path)
{
	...
	DWORD ret = GetNamedSecurityInfo((LPCSTR)path, SE_FILE_OBJECT, DACL_SECURITY_INFORMATION, nullptr, nullptr, &old_acl, nullptr, &sd);
	...
	{
		SetNamedSecurityInfo((LPSTR)path, SE_FILE_OBJECT, siInfo, NULL, NULL, new_acl, NULL);
		LocalFree(new_acl);
	}
...
int test_wmain(int argc, wchar_t *argv[])
{
	if (argc != 2)
	{
	...
	printf("Found a matching process with PID %lu! Injecting hook_test64 ... ", pid);

	// Wait just a little bit for the application to initialize
	Sleep(50);

	// Open target application process
	const scoped_handle remote_process = OpenProcess(PROCESS_CREATE_THREAD | PROCESS_VM_OPERATION | 
	...
	wcscat_s(arg.load_path, remote_is_wow64 ? L"hook_test32.dll" : L"hook_test64.dll");
	...
	{
#if RESHADE_LOADING_THREAD_FUNC
		printf("\nFailed to load hook_test64 in target application! Error code is 0x%x.\n", exit_code);
		return exit_code;
#else
		printf("\nFailed to load hook_test64 in target application!\n");
