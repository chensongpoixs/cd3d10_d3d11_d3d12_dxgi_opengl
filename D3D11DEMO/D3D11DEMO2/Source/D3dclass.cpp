...
bool D3DClass::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	unsigned long long stringLength;			// using "size_t" here may be better because "unsigned long long" only suit for x64
	...
	// Create a DirectX grahics interface factory.
	...
	result = factory->EnumAdapters(g_gpu_index, &adapter);
	if (FAILED(result))
	{
		return false;
	}
	...
	adapter->GetDesc(&adapterDesc1);
	wcstombs(desc, adapterDesc1.Description, sizeof(desc));
	if (strstr(desc, "NVIDIA") != NULL)
	{
		...
		if (out_file_ptr)
		{
			...
			fprintf(out_file_ptr, "use gpu info [desc = %s] g_gpu_index = %u ok \n", desc, g_gpu_index);
			fflush(out_file_ptr);
...
void D3DClass::BeginScene(float red, float green, float blue, float alpha)
{
	...
	m_deviceContext->ClearDepthStencilView(m_depthStencilView, D3D11_CLEAR_DEPTH, 1.0, 0);
