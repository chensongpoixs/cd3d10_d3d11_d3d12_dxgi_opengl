...
static struct d3d12_data data = {0};
...
void d3d12_free(void)
{
	...
	DEBUG_EX_LOG("----------------- d3d12 capture freed ----------------");
}
...
static bool create_d3d12_tex(bb_info &bb)
{
	...
	for (UINT i = 0; i < bb.count; i++) 
	{
		...
		if (FAILED(hr)) 
		{
			WARNING_EX_LOG("create_d3d12_tex: failed to create backbuffer11", hr);
			return false;
		}
	}
	...
	desc11.MipLevels = 1;
	desc11.ArraySize = 1;
	desc11.Format = apply_dxgi_format_typeless(data.format, false);
	...
	if (FAILED(hr)) 
	{
		WARNING_EX_LOG("create_d3d12_tex: creation of d3d11 copy tex failed",
			hr);
		return false;
	}
	...
	if (FAILED(hr)) {
		WARNING_EX_LOG("create_d3d12_tex: failed to query "
	...
	if (FAILED(hr)) 
	{
		WARNING_EX_LOG("create_d3d12_tex: failed to get shared handle", hr);
		return false;
	}
...
static bool d3d12_init_11on12(ID3D12Device *device)
{
	...
	if (!initialized_11 && !d3d11) 
	{
		...
		if (!d3d11) 
		{
			WARNING_EX_LOG("d3d12_init_11on12: failed to load d3d11");
		}
		initialized_11 = true;
	}
	...
	if (!initialized_func && !create_11_on_12) 
	{
		...
		if (!create_11_on_12)
		{
			WARNING_EX_LOG("d3d12_init_11on12: Failed to get "
	...
	if (!create_11_on_12) 
	{
		WARNING_EX_LOG("create_11_on_12 false !!!");
		return false;
	}
	...
	{
		DEBUG_EX_LOG("d3d12_init_11on12: creating 11 device: queue=0x%" PRIX64,
		...
		DEBUG_EX_LOG("d3d12_init_11on12: failed to create 11 device", hr);
	}
	...
	if (FAILED(hr)) 
	{
		WARNING_EX_LOG("d3d12_init_11on12: failed to query 11on12 device", hr);
		return false;
	}
...
static bool d3d12_shtex_init(ID3D12Device *device, HWND window, bb_info &bb)
{
	...
	//(HWND window, uint32_t cx, uint32_t cy, uint32_t format, HANDLE handle)

	capture_init_shtex(window, data.cx, data.cy, data.format, data.handle);
	DEBUG_EX_LOG("d3d12 shared texture capture successful");
	return true;
}
				     ...
				     bb_info &bb)
{
	...
	if (FAILED(hr)) {
		WARNING_EX_LOG("d3d12_init_format: swap->GetDesc failed", hr);
		return false;
	}
	...
	DEBUG_EX_LOG("[format = %d][Width = %u][Height = %u]", desc.BufferDesc.Format, desc.BufferDesc.Width, desc.BufferDesc.Height);
	hr = swap->QueryInterface(&swap3);
	if (SUCCEEDED(hr)) {
		data.dxgi_1_4 = true;
		DEBUG_EX_LOG("We're DXGI1.4 boys!");
		swap3->Release();
	}

	DEBUG_EX_LOG("Buffer count: %d, swap effect: %d", (int)desc.BufferCount,
	     (int)desc.SwapEffect);
	...
	if (bb.count > MAX_BACKBUFFERS) {
		DEBUG_EX_LOG("Somehow it's using more than the max backbuffers.  "
...
static void d3d12_init(IDXGISwapChain *swap)
{
	...
	if (SUCCEEDED(hr)) 
	{
		DEBUG_EX_LOG("d3d12_init: device=0x%" PRIX64, (uint64_t)(uintptr_t)device);
		...
		if (d3d12_init_format(swap, window, bb)) 
		{
			...
			{
				DEBUG_EX_LOG("d3d12_init: shared memory capture currently "
				     "unsupported; ignoring");
			}*/

			if (!d3d12_shtex_init(device, window, bb))
			{
				ERROR_EX_LOG("d3d12 shtex init failed  !!!");
				d3d12_free();
			}
			else
			{
				DEBUG_EX_LOG("d3d12 shtex init OK !!!");
		...
		WARNING_EX_LOG("d3d12_init: failed to get device from swap", hr);
	}
}
...
{
	DEBUG_EX_LOG("ExecuteCommandLists callback: queue=0x%" PRIX64,
	...
	if (dxgi_possible_swap_queue_count <
		...
		if ((dxgi_presenting > 0) &&
			...
			if (try_append_queue_if_unique(queue)) {
				DEBUG_EX_LOG("Remembering D3D12 queue from present: queue=0x%" PRIX64,
				     (uint64_t)(uintptr_t)queue);
			}
			...
			if (try_append_queue_if_unique(queue)) {
				DEBUG_EX_LOG("Remembering D3D12 queue from first direct submit after present: queue=0x%" PRIX64,
				     (uint64_t)(uintptr_t)queue);
			}
		} else {
			DEBUG_EX_LOG("Ignoring D3D12 queue=0x%" PRIX64,
				     (uint64_t)(uintptr_t)queue);
		}
	}
			 ...
			 PFN_ExecuteCommandLists *execute_command_lists_addr)
{
	PFN_D3D12_CREATE_DEVICE create =
		(PFN_D3D12_CREATE_DEVICE)GetProcAddress(d3d12_module,
							"D3D12CreateDevice");
	if (!create) {
		DEBUG_EX_LOG("Failed to load D3D12CreateDevice");
		return false;
	}
	...
	if (SUCCEEDED(create(NULL, D3D_FEATURE_LEVEL_11_0,
		...
		if (success) {
			...
			*execute_command_lists_addr =
			...
			DEBUG_EX_LOG("Failed to create D3D12 command queue");
		}

		device->Release();
	} else {
		DEBUG_EX_LOG("Failed to create D3D12 device");
	}
...
bool hook_d3d12(void)
{
	...
	if (!d3d12_module) 
	{
		WARNING_EX_LOG( "Failed to find d3d12.dll. Skipping hook attempt.");
		return false;
	}
	...
	{
		WARNING_EX_LOG("Failed to get D3D12 values");
		return true;
	}

	if (!execute_command_lists_addr)
	{
		WARNING_EX_LOG("Invalid D3D12 values");
		return true;
	}
	...
	if (success) {
		DEBUG_EX_LOG("Hooked ID3D12CommandQueue::ExecuteCommandLists");
		DEBUG_EX_LOG("Hooked D3D12");
	} else {
