...
static inline bool shex_init_d3d11()
{
	...
	if (!d3d11) {
		ERROR_EX_LOG("d3d9_init: Failed to load D3D11");
		return false;
	}

	dxgi = load_system_library("dxgi.dll");
	if (!dxgi) {
		ERROR_EX_LOG("d3d9_init: Failed to load DXGI");
		return false;
	}
	...
	if (!create_factory) {
		ERROR_EX_LOG("d3d9_init: Failed to get CreateDXGIFactory1 address");
		return false;
	}
	...
	if (!create_device) {
		ERROR_EX_LOG("d3d9_init: Failed to get D3D11CreateDevice address");
		return false;
	}

	hr = create_factory(__uuidof(IDXGIFactory1), (void **)&factory);
	if (FAILED(hr)) {
		ERROR_EX_LOG("d3d9_init: Failed to create factory object", hr);
		return false;
	}
	...
	if (FAILED(hr)) {
		ERROR_EX_LOG("d3d9_init: Failed to get adapter", hr);
		return false;
	}
	...
	if (FAILED(hr)) {
		ERROR_EX_LOG("d3d9_init: Failed to create D3D11 device", hr);
		return false;
	}
...
static inline bool d3d9_shtex_init_shtex()
{
	...
	if (FAILED(hr)) {
		ERROR_EX_LOG("d3d9_shtex_init_shtex: Failed to create D3D11 texture",
			hr);
		return false;
	}
	...
	hr = data.d3d11_tex->QueryInterface(__uuidof(IDXGIResource),
					    (void **)&res);
	if (FAILED(hr)) {
		ERROR_EX_LOG("d3d9_shtex_init_shtex: Failed to query IDXGIResource",
			hr);
		return false;
	}
	...
	if (FAILED(hr)) {
		ERROR_EX_LOG("d3d9_shtex_init_shtex: Failed to get shared handle",
			hr);
		return false;
	}
...
{
	struct d3d9_offsets offsets = g_graphics_offsets->d3d9;
	...
	if (FAILED(hr)) {
		ERROR_EX_LOG("d3d9_shtex_init_copytex: Failed to create shared texture",
			hr);
		return false;
	}
	...
	if (FAILED(hr)) {
		ERROR_EX_LOG("d3d9_shtex_init_copytex: Failed to get surface level",
			hr);
		return false;
	}
...
static bool d3d9_shmem_init_buffers(size_t buffer)
{
	...
	if (FAILED(hr)) {
		ERROR_EX_LOG("d3d9_shmem_init_buffers: Failed to create surface",
			hr);
		return false;
	}
	...
	if (buffer == 0) {
		...
		hr = data.copy_surfaces[buffer]->LockRect(&rect, nullptr,
							  D3DLOCK_READONLY);
		if (FAILED(hr)) {
			ERROR_EX_LOG("d3d9_shmem_init_buffers: Failed to lock "
				"buffer",
				hr);
			return false;
		}
	...
	hr = data.device->CreateQuery(D3DQUERYTYPE_EVENT,
				      &data.queries[buffer]);
	if (FAILED(hr)) {
		ERROR_EX_LOG("d3d9_shmem_init_buffers: Failed to create query", hr);
		return false;
	}
...
static bool d3d9_get_swap_desc(D3DPRESENT_PARAMETERS &pp)
{
	...
	if (FAILED(hr)) {
		ERROR_EX_LOG("d3d9_get_swap_desc: Failed to get swap chain", hr);
		return false;
	}
	DEBUG_EX_LOG("");
	...
	if (FAILED(hr)) {
		ERROR_EX_LOG("d3d9_get_swap_desc: Failed to get "
...
static bool d3d9_init_format_backbuffer(HWND &window)
{
	...
	if (FAILED(hr)) {
		ERROR_EX_LOG("d3d9_init_format_backbuffer: Failed to get "
...
static void d3d9_init(IDirect3DDevice9 *device)
{
	...
	bool has_d3d9ex_bool_offset = g_graphics_offsets->d3d9.d3d9_clsoff && g_graphics_offsets->d3d9.is_d3d9ex_clsoff;
	bool success;
	HWND window = nullptr;
	HRESULT hr;
	DEBUG_EX_LOG("");
				 ...
				 IDirect3DSurface9 *&backbuffer)
{
	...
	if (!present_recurse) {
		...
		if (FAILED(hr)) {
			ERROR_EX_LOG("d3d9_shmem_capture: Failed to get "
				"backbuffer",
				hr);
		}
...
static void setup_reset_hooks(IDirect3DDevice9 *device)
{
	...
	if (success) {
		DEBUG_EX_LOG("Hooked IDirect3DDevice9::Reset");
		if (RealResetEx)
			DEBUG_EX_LOG("Hooked IDirect3DDevice9Ex::ResetEx");
		hooked_reset = true;
	} else {
		RealReset = nullptr;
		RealResetEx = nullptr;
				    ...
				    void **present_swap_addr)
{
	...
	DEBUG_EX_LOG("D3D9 values invalid, manually obtaining");

	create_ex = (d3d9create_ex_t)GetProcAddress(d3d9_module,
						    "Direct3DCreate9Ex");
	if (!create_ex) 
	{
		ERROR_EX_LOG("Failed to load Direct3DCreate9Ex");
		return false;
	}
	if (FAILED(create_ex(D3D_SDK_VERSION, &d3d9ex))) {
		ERROR_EX_LOG("Failed to create D3D9 context");
		return false;
	}
	...
	if (SUCCEEDED(hr)) {
		...
		*present_ex_addr = (void *)vtable[121];
		...
		ERROR_EX_LOG("Failed to create D3D9 device");
		return false;
	}
...
bool hook_d3d9(void)
{
	...
	else 
	{
		...
		if (!manually_get_d3d9_addrs(d3d9_module, &present_addr,
					     ...
					     &present_swap_addr)) 
		{
			ERROR_EX_LOG("Failed to get D3D9 values");
			return true;
		}
	}
	...
	{
		ERROR_EX_LOG("Invalid D3D9 values");
		return true;
	}
	...
	if (success) {
		if (RealPresentSwap)
			DEBUG_EX_LOG("Hooked IDirect3DSwapChain9::Present");
		if (RealPresentEx)
			DEBUG_EX_LOG("Hooked IDirect3DDevice9Ex::PresentEx");
		if (RealPresent)
			DEBUG_EX_LOG("Hooked IDirect3DDevice9::Present");
		DEBUG_EX_LOG("Hooked D3D9");
	} else {
