...
FILE* out_Appwinrt_capture_ptr = NULL;
#define WRITE_FILE()                                                          \
	if (!out_Appwinrt_capture_ptr) {                                         \
		out_Appwinrt_capture_ptr = fopen("Appwinrt_capture.log", "wb+");    \
	}                                                                     \
	fprintf(out_Appwinrt_capture_ptr, "[%s][%d]\n", __FUNCTION__, __LINE__); \
	fflush(out_Appwinrt_capture_ptr);

#define WRITE_ERROR_FILE()                                                 \
	if (!out_Appwinrt_capture_ptr) {                                      \
		out_Appwinrt_capture_ptr = fopen("Appwinrt_capture.log", "wb+"); \
	}                                                                  \
	fprintf(out_Appwinrt_capture_ptr, "[%s][%d][ERROR]\n", __FUNCTION__,  \
		__LINE__);                                                 \
	fflush(out_Appwinrt_capture_ptr);
namespace util
{
...
bool App::init(/*winrt::ContainerVisual root,*/ winrt::GraphicsCapturePicker capturePicker, winrt::FileSavePicker savePicker)
{
  ...
  /*  m_mainThread = winrt::DispatcherQueue::GetForCurrentThread();
    ...
    winrt::com_ptr<ID3D11Device> d3dDevice = util::CreateD3DDevice();
    winrt::com_ptr<IDXGIDevice>  dxgiDevice = d3dDevice.as<IDXGIDevice>();
    m_device = CreateDirect3DDevice(dxgiDevice.get());
    /*winrt::com_ptr<ID3D11DeviceContext> deviceCtx;
    d3dDevice->GetImmediateContext(deviceCtx.put());*/
